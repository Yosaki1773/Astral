    ...
    <div class="record-detail-page">
            ...
            <div class="status-bar-content">
                ...
                <div class="status-bar-left">
                    ...
                    <div class="identity-block">
                        <div class="identity-top">
                            ...
                            <span v-if="recordStore.currentRecord" class="status-badge" :class="`status-${recordStore.currentRecord.status || 'default'}`">
                                <span class="status-dot"></span>
                                {{ getStatusBadgeText(recordStore.currentRecord.status) }}
                            </span>
                        </div>
                        <div v-if="recordStore.currentRecord" class="identity-sub">
                            <span class="record-id-tag">#{{ recordStore.currentRecord.id }}</span>
                            <span class="bullet">•</span>
                            <span class="created-time">{{ formatDate(recordStore.currentRecord.created_at) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="recordStore.currentRecord" class="status-bar-meta">
                    ...
                    <div class="meta-col">
                        ...
                        <div class="meta-user">
                            <span class="user-avatar-mini">{{ (recordStore.currentRecord.user_name || 'U').charAt(0).toLowerCase() }}</span>
                            <span class="meta-value">{{ recordStore.currentRecord.user_name || '-' }}</span>
                        </div>
                    </div>
                    <!-- 接入模型 -->
                    <div class="meta-col">
                        <span class="micro-label">接入模型</span>
                        <span class="meta-value font-mono font-semibold" :title="recordStore.currentRecord.model_name || '-'">
                    ...
                    <div class="meta-col">
                        <div class="meta-label-flex">
                            ...
                            <span v-if="recordStore.currentRecord.vendor_name" class="vendor-tag">
                                {{ recordStore.currentRecord.vendor_name }}
                            </span>
                        </div>
                        <span class="meta-value font-mono font-secondary truncate" :title="recordStore.currentRecord.vendor_model_name || '-'">
                    ...
                    <div class="meta-col">
                        ...
                        <div v-if="recordStore.currentRecord.client_format" class="protocol-badge-row">
                            <span class="protocol-badge">{{ recordStore.currentRecord.client_format.toUpperCase() }}</span>
                            <template v-if="recordStore.currentRecord.upstream_format">
                                <span class="protocol-arrow">→</span>
                                <span class="protocol-badge protocol-upstream">{{ recordStore.currentRecord.upstream_format.toUpperCase() }}</span>
                            </template>
                        </div>
                ...
                <div v-if="recordStore.currentRecord" class="status-bar-metrics">
                    ...
                    <div class="metric-col">
                        ...
                        <div v-if="usageTokens" class="metric-value-row font-mono">
                            ...
                            <span v-if="usageTokens.cacheReadTokens !== null" class="token-cached" title="缓存命中扩展 Token">
                    ...
                    <div class="metric-col">
                        ...
                        <div v-if="usageTokens?.cacheHitRate != null" class="cache-hit-row">
                            <span class="cache-hit-num">{{ usageTokens.cacheHitRate.toFixed(1) }}%</span>
                    ...
                    <div class="metric-col">
                        ...
                        <span v-if="recordStore.currentRecord.first_token_latency" class="ttft-pill font-mono">
                            {{ recordStore.currentRecord.first_token_latency.toLocaleString() }}ms
                        </span>
                        <span v-else class="meta-value font-mono">-</span>
                    </div>
                </div>
        ...
        <main class="workbench-container">
            <a-spin :spinning="recordStore.loading">
                        ...
                        <section class="panel-card left-panel-card">
                            ...
                            <div class="panel-header">
                                ...
                                <div class="panel-actions">
                                    ...
                                    <button
                                        ...
                                        class="btn-pill-action"
                                        title="复制完整请求JSON"
                                        @click="copyText(recordStore.currentRecord.request_data || '', '请求 JSON')"
                                    >
                                        <CopyOutlined />
                                        <span>Copy JSON</span>
                                    </button>
                            ...
                            <div v-if="parsedRequestParams" class="params-strip">
                                <div class="params-header">
                                    <span class="params-title">调用运行时参数 (INFERENCE HYPERPARAMETERS)</span>
                                    <span class="params-route font-mono">{{ requestRouteInfo }}</span>
                                </div>
                                <div class="params-grid">
                                    <div class="param-box">
                                        <span class="param-label">temperature</span>
                                        <span class="param-val">{{ parsedRequestParams.temperature ?? '-' }}</span>
                                    </div>
                                    <div class="param-box">
                                        <span class="param-label">stream</span>
                                        <span class="param-val" :class="parsedRequestParams.stream ? 'text-purple' : ''">
                                            {{ parsedRequestParams.stream != null ? String(parsedRequestParams.stream) : '-' }}
                                        </span>
                                    </div>
                                    <div class="param-box">
                                        <span class="param-label">max_tokens</span>
                                        <span class="param-val">{{ parsedRequestParams.max_tokens ?? '-' }}</span>
                                    </div>
                                    <div class="param-box">
                                        <span class="param-label">top_p</span>
                                        <span class="param-val">{{ parsedRequestParams.top_p ?? '-' }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="viewer-iframe-wrapper">
                                <div v-if="messageCount === 0" class="no-payload-hint">
                                    <div v-if="!recordStore.currentRecord.request_data" class="no-payload-title">请求内容未记录</div>
                                    <div v-else class="no-payload-title">暂无可供可视化的对话数据</div>
                                    <div class="no-payload-desc">如需记录请到设置中打开开关，或在右侧查看原始 JSON</div>
                                </div>
                                <iframe
                                    v-else
                                    ref="viewerIframe"
                        ...
                        <section class="panel-card right-panel-card">
                            ...
                            <div class="panel-header">
                                <div class="panel-title-wrap">
                                    ...
                                    <h2 class="panel-title">响应预览与日志 (RESPONSE &amp; LOGS)</h2>
                                </div>
                            ...
                            <div v-show="activeRightTab === 'preview'" class="right-tab-content preview-scrollable">
                                <!-- 1. 响应元数据 & 状态条 (Response Status Strip) -->
                                <div class="response-status-strip">
                                    <div class="response-strip-left">
                                        <span class="res-status-tag" :class="`tag-${recordStore.currentRecord.status || 'default'}`">
                                            {{ getResponseStatusText(recordStore.currentRecord.status) }}
                                        </span>
                                        <span v-if="parsedResponseMeta.finishReason" class="res-meta-pill">
                                            Finish: <strong>{{ parsedResponseMeta.finishReason }}</strong>
                                        </span>
                                        <span v-if="parsedResponseMeta.chunkCount" class="res-meta-pill">
                                            Chunks: <strong>{{ parsedResponseMeta.chunkCount }}</strong>
                                        </span>
                                    </div>
                                    <div v-if="generationSpeed !== null" class="response-strip-right font-mono">
                                        <span class="gen-speed-label">生成速率:</span>
                                        <span class="gen-speed-val">{{ generationSpeed }} tokens/s</span>
                                    </div>
                                </div>
                                ...
                                <div v-if="latencyBreakdown" class="latency-section">
                                    <div class="latency-header">
                                        ...
                                        <span class="latency-total font-mono">
                                            总历时: <strong class="total-num">{{ latencyBreakdown.totalDuration }}ms</strong>
                                        </span>
                                    </div>
                                    <!-- 多段彩色延时进度条 -->
                                    ...
                                    <div class="latency-legend font-mono">
                                        <div class="legend-item text-slate-500">
                                            ...
                                            <span class="truncate">DNS/网络: {{ latencyBreakdown.dnsMs }}ms</span>
                                        </div>
                                        <div class="legend-item text-amber-600 font-medium">
                                            <span class="legend-dot dot-amber"></span>
                                            <span class="truncate">TTFT: {{ latencyBreakdown.ttftMs }}ms</span>
                                        </div>
                                        <div class="legend-item text-emerald-600 font-medium">
                                            <span class="legend-dot dot-emerald"></span>
                                            <span class="truncate">传输流耗时: {{ latencyBreakdown.streamMs }}ms</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="assistant-content-container">
                                    ...
                                    <div v-if="assistantReplyText" class="assistant-markdown-body">
                                        <div class="rendered-markdown" v-html="renderMarkdown(assistantReplyText)"></div>
                                    </div>
                                    <div v-else-if="!recordStore.currentRecord.response_data" class="no-payload-hint">
                                    ...
                                    <div v-else class="no-payload-hint">
                                        ...
                                        <div class="no-payload-desc">请通过“响应 JSON”标签页查看返回的数据结构</div>
                                    </div>

                                    <!-- 底部 Chunk 流式结束指示 -->
                                    <div v-if="assistantReplyText && parsedResponseMeta.finishReason" class="stream-done-pill font-mono">
                                        <span>[DONE] · {{ parsedResponseMeta.chunkCount ? `${parsedResponseMeta.chunkCount} chunks aggregated · ` : '' }}Final finish_reason: "{{ parsedResponseMeta.finishReason }}"</span>
                                    </div>
                                </div>
                            </div>

                            <!-- Tab 2: 响应 JSON (树状浏览) -->
                            <div v-show="activeRightTab === 'response_json'" class="right-tab-content json-scrollable">
                                <div class="json-pane-header">
                                    ...
                                    <div class="json-pane-actions">
                                        ...
                                        <a-button type="link" size="small" @click="responseJsonRef?.handleCopy ? responseJsonRef.handleCopy() : copyText(recordStore.currentRecord.response_data || '', '响应 JSON')">
                                            复制
                                        </a-button>
                                        <a-button
                                            type="link"
                            ...
                            <!-- Tab 3: 请求 JSON (树状浏览) -->
                            <div v-show="activeRightTab === 'request_json'" class="right-tab-content json-scrollable">
                                <div class="json-pane-header">
                                    ...
                                    <div class="json-pane-actions">
                                        ...
                                        <a-button type="link" size="small" @click="requestJsonRef?.handleCopy ? requestJsonRef.handleCopy() : copyText(recordStore.currentRecord.request_data || '', '请求 JSON')">
                                            复制
                                        </a-button>
                                        <a-button
                                            type="link"
                            ...
                            <!-- Tab 4: 分块流 (SSE) 原始视图 -->
                            <div v-show="activeRightTab === 'sse_stream'" class="right-tab-content sse-scrollable">
                                <div class="sse-pane-header">
                                    <span class="font-mono text-xs font-semibold text-slate-700">Server-Sent Events 模拟/还原分块流</span>
                                    <span class="font-mono text-[11px] text-slate-400">Content-Type: text/event-stream</span>
                                </div>
                            ...
                            <!-- Tab 5: 活动日志 (Timeline) -->
                            <div v-show="activeRightTab === 'activity'" class="right-tab-content activity-scrollable">
                                <ActivityTimeline :activities="recordStore.activities" />
                            </div>

                            <!-- Tab 6: 报错信息 (Error) -->
                            <div v-show="activeRightTab === 'error'" class="right-tab-content error-scrollable">
                                <div class="error-pane-card">
                                    <div v-if="recordStore.currentRecord.failed_code" class="error-badge-row">
                                    ...
                                    <div class="error-box">
                                        <pre class="error-text">{{ getErrorMessage(recordStore.currentRecord.response_data) }}</pre>
                                    </div>
                                </div>
                            </div>
                ...
                <a-empty v-else-if="recordStore.recordNotFound" description="请求未找到" class="empty-holder" />
                <a-empty v-else-if="recordStore.recordLoadFailed" description="请求详情加载失败，请稍后重试" class="empty-holder" />
            </a-spin>
        </main>
    </div>
</template>
...
const isRawRequestOpen = ref(false);
const activeRightTab = ref<'preview' | 'response_json' | 'request_json' | 'sse_stream' | 'activity' | 'error'>('preview');
...
const requestRouteInfo = computed(() => {
    const protocol = recordStore.currentRecord?.client_format?.toLowerCase() || 'openai';
...
const assistantReplyText = computed<string | null>(() => {
    ...
    if (Array.isArray(res.choices) && res.choices.length > 0) {
        ...
        if (choice.message?.content) {
            return typeof choice.message.content === 'string' ? choice.message.content : JSON.stringify(choice.message.content, null, 2);
        }
    ...
    if (res.output && Array.isArray(res.output)) {
        ...
        if (converted.length > 0) {
            return converted.map((m: any) => (typeof m.content === 'string' ? m.content : JSON.stringify(m.content))).join('\n');
        }
    }

    if (res.message?.content) {
        return typeof res.message.content === 'string' ? res.message.content : JSON.stringify(res.message.content, null, 2);
    }
...
const latencyBreakdown = computed(() => {
    ...
    const ttft = recordStore.currentRecord?.first_token_latency || Math.min(total, 120);
...
function getStatusBadgeText(status: string | null | undefined): string {
    switch (status) {
        case 'success': return '成功 200 OK';
        case 'failed': return '请求失败';
        case 'processing': return '处理中';
        case 'init': return '已初始化';
        default: return '未知状态';
    }
}
...
function getResponseStatusText(status: string | null | undefined): string {
    switch (status) {
        case 'success': return 'HTTP 200 OK';
        case 'failed': return 'HTTP 500 / FAILED';
...
.rendered-markdown :deep(h1) { font-size: 15px; }
.rendered-markdown :deep(h2) { font-size: 14px; }
.rendered-markdown :deep(h3) { font-size: 13px; }
.rendered-markdown :deep(h4) { font-size: 12.5px; }
